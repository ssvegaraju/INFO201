...
server <- function(input, output) {
  ...
  output$description <- renderText({
    ...
    str <- paste0(
      "The correlation coefficient between Deaths and Migrations ",
      "for <em>", input$region, "</em> is <b>", r, "</b>."
    )
    if (r > 0.5) {
      str <- paste0(
        str, " This implies that there is a <b>decently strong</b>",
      ...
      str <- paste0(
        str, " This implies that there is a <b>very weak</b>",
  ...
  output$goal <- renderText({
    str <- paste0(
      "The goal of this visualization is to find if migrations",
  ...
  output$plot_description <- renderText({
    if (input$first_state != input$second_state) {
      ...
      starter_sentence <- paste0(
        "The correlation coefficent between ", input$first_state, " and ",
